...
model_types = ["CNT","NAIVE","BATCH_EQ"]
# model_types = ["NAIVE","BATCH_EQ"]
model_names = dict(
                CNT=["raw", "pcs"], 
                # NAIVE=["scvi_emb", "scvi_expr", "scvi_expr_pcs", "scanvi"], 
                # BATCH_EQ=["scvi_emb", "scvi_expr", "scvi_expr_pcs", "scanvi"]
                NAIVE=["scvi_emb", "scvi_expr", "scanvi"], 
                BATCH_EQ=["scvi_emb", "scvi_expr", "scanvi"]               
                )
...
model = model_names[model_type][0]
model_path = MODEL_ROOT / set_name / model_type
model_name = model
output_data_path = train_path / "LABELATOR" 
artifacts_path = Path(f"artifacts/{set_name}/{model_type}/")
print(f"training:: {train_path=}, {model_path=}, {model_name=}, {output_data_path=}, {artifacts_path=}, {gen_plots=}, {retrain_model=}, {labels_key=}")
...
for set_name, n_top_gene in zip(ds_names, ns_top_genes):
    train_path = Path(f"{XYLENA2_PATH}{set_name}/{XYLENA2_TRAIN}")

    for model in model_names[model_type]:
        model_path = MODEL_ROOT / set_name / model_type
        model_name = model
        output_data_path = Path(f"{XYLENA2_PATH}{set_name}/LABELATOR/")
        artifacts_path = Path(f"artifacts/{set_name}/{model_type}/")
        print(f"training:: {train_path=}, {model_path=}, {model_name=}, {output_data_path=}, {artifacts_path=}, {gen_plots=}, {retrain_model=}, {labels_key=}")
        train_lbl8r(train_path, 
                    model_path, 
                    model_name,
                    output_data_path,
                    artifacts_path,
