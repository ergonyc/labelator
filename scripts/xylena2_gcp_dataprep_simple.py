...
for samp in usamples:
    ...
    # note: No fully labeled samples.. only unlabeled and partially labeled
...
# In[ ]:
newmeta["train_sample"] = newmeta["sample"].isin(train_samples)
newmeta["test_sample"] = newmeta["sample"].isin(test_samples)
# In[ ]:
newmeta["train"] = newmeta["train_sample"] & newmeta["tt"]
newmeta["test"] = newmeta["test_sample"] & newmeta["tt"]

# make two query subsets... ones from labeled samples
# and one which don't have any labeled samples

newmeta["queryA"] = newmeta["partlabeled_sample"] & newmeta["query"]
newmeta["queryB"] = newmeta["unlabeled_sample"] & newmeta["query"]
...
# In[ ]: rename these to scanpy standards
...
query_ad = raw_ad[raw_ad.obs["queryA"]]
# In[ ]:
raw_query_filen = data_path / XYLENA2_QUERY_A
...
raw_ad = ad.read_h5ad(full_filen, backed="r")
# In[]
query_ad = raw_ad[raw_ad.obs["queryB"]]
# In[ ]:
raw_query_filen = data_path / XYLENA2_QUERY_B
query_ad.write_h5ad(raw_query_filen)
del query_ad
...
for ds_name, n_top_gene in zip(ds_names, ns_top_genes):
    ...
    query_filen = ds_path / XYLENA2_QUERY_A
...
for ds_name, n_top_gene in zip(ds_names, ns_top_genes):
    ...
    query_filen = ds_path / XYLENA2_QUERY_B
    query_ad.write_h5ad(query_filen)
    print(f"Saved {query_filen}")
del query_ad
...
# make sure the data is sparse
ds_path = data_path / "2k"
raw_test_filen = ds_path / XYLENA2_TEST
